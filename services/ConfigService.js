...
export default class ConfigService extends BaseService {
  set = async (params, msgEvent, commands, config, botClient) => {
    const guildId = msgEvent.guild.id;
    let { key, value } = params;

    // Ensure that prefix ends with "!"
    if (key === "prefix" && !value.endsWith("!")) value = value + "!";

    store.dispatch(add(guildId, key, value));
    msgEvent.reply(`Setting **"${key}"** has now value **"${value}"**`);
  };

  get = async (params, msgEvent, commands, config, botClient) => {
    const [key] = params;
    const guildId = msgEvent.guild.id;
    ...
    if (configuration.hasOwnProperty(key)) {
      msgEvent.reply(
        `Setting **"${key}"** has value **"${configuration[key]}"**`
      );
    } else {
